...
!-----------------------------------------------------------------------
SUBROUTINE recursive_filter_ad
   ...
   INTEGER(i4)    :: k, ione,  iter,init, niter
...
! Average
      IF ( drv%mask(drv%ktr) .GT. 1 ) THEN
         IF ( eta_condition ) THEN
            grd%eta_ad(:,:) = (grd%eta_ad(:,:  ) + grd%eta(:,:  ) ) * 0.5_r8
         ENDIF
         grd%tem_ad(:,:,:)   = (grd%tem_ad(:,:,:) + grd%tem(:,:,:) ) * 0.5_r8
         grd%sal_ad(:,:,:)   = (grd%sal_ad(:,:,:) + grd%sal(:,:,:) ) * 0.5_r8
      ENDIF

   ENDDO  !iter

END SUBROUTINE recursive_filter_ad
