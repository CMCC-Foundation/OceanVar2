...
!-----------------------------------------------------------------------
!                                                                      !
!> Write outputs and diagnostics for XBT
!!
!!
!!
!                                                                      !
...
SUBROUTINE wrt_xbt
   ...
   ALLOCATE ( allflg(xbt%no), allinc(xbt%no),  alleve(xbt%no) )
   ...
   CALL MPI_REDUCE( xbt%flc, allflg, nobs, mpi%i8  ,   &
                    MPI_MAX, 0, mpi%comm, ierr)
   CALL MPI_REDUCE( xbt%eve, alleve, nobs, mpi%i8  ,   &
                    MPI_MAX, 0, mpi%comm, ierr)
   ...
   IF ( mpi%myrank .EQ. 0 ) THEN
      ...
      xbt%eve(1:xbt%no) = alleve(1:xbt%no)

      ! Create file
      stat = NF90_CREATE('obsstat_xbt.nc', NF90_SHARE, ncid)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_OPEN  ('obsstat_xbt.nc', NF90_WRITE, ncid)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      ...
      ! Fill in variables
      stat = NF90_PUT_VAR(ncid, varid(1), xbt%lon)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(2), xbt%lat)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(3), xbt%tim)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(4), xbt%ino)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(5), xbt%par)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(6), xbt%flg)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(7), xbt%eve)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(8), xbt%val)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(9), xbt%bac)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(10),xbt%err)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(11),xbt%res)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(12),xbt%bia)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(13),xbt%inc)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
      stat = NF90_PUT_VAR(ncid, varid(14),xbt%dpt)
      IF (stat /= NF90_NOERR) CALL netcdf_err(stat)
